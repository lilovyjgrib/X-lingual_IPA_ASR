...
We trained an audio to IPA ResNet–BiLSTM CTC recognizer on TIMIT only (\texttt{0.0339} on English validation set), taking inspiration from \citet{dhakal2022automatic},
...
we applied a modified version of feature-weighted Levenshtein algorithm for evaluation. The system reaches \approx\texttt{0.34} PER on Yoruba (median \texttt{0.30}).
...
labio-velar stops are often perceived as \ipa{/w/}. At the same time, the model over-generates, partly because of pauses outside of utterance and extra-linguistic noise in the evaluation set, partly because the transfer keeps the tendency for
